...
public class MainActivity extends AppCompatActivity {
    ...
    private View.OnClickListener listener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            try {
                searchNumber = Integer.parseInt(search.getText().toString()); // считывание введённого текста
            } catch (Exception e){}

            mapPoem = stringMapPoem(poem); // формирование мап подстрок поэмы

            if (searchNumber != null) {
                String resultSearch = searchMapPoem(searchNumber, mapPoem); // поиск String searchText в мап
                if (resultSearch == null) { // если в поэме ничего не найдено, то
                    output.setText("Строки с данным количеством символов в поэме не найдено");
                } else {
                    output.setText("В поэме найдено " + resultSearch);
                }
            } else {
                output.setText("Возникла ошибка.");
            }
        }
    };

    // метод поиска информации в мап
    private String searchMapPoem(Integer search, Map<Integer, String> map) {

        String result = "";
        int count = 0;

        for (Integer key: map.keySet()) { // перебор мап по ключам
            if(map.get(key).length() == search) { // если в строке с ключом key имеется фраза поиска, то
        ...
        if (count == 0) {
            ...
            return count + " строк с данной длиной:" + result; // вывод количества найденных в поэме фраз и подстрок поэмы, где они были найдены
        }
    }
